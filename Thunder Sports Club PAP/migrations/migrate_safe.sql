...
-- Limpar duplicados de cont_socio (guardar apenas o registo mais antigo)
DELETE FROM socios WHERE num_socio NOT IN (
    SELECT MIN(num_socio) FROM (
        SELECT MIN(num_socio) as num_socio FROM socios WHERE cont_socio IS NOT NULL GROUP BY cont_socio
    ) AS min_records
) AND cont_socio IS NOT NULL;

-- Agora sim, adicionar índices
